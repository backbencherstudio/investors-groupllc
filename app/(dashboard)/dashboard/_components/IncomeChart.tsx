    ...
    return (
      <div className="bg-white p-4 rounded-xl shadow-lg border text-sm">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          />
          <span>ROI</span>
          <span className="ml-auto font-semibold">{monthData?.roi || 0}%</span>
        </div>
      </div>
    );
  }
...
export default function IncomeChart() {
  ...
  const totalRoi = useMemo(() => {
    ...
    const avgRoi =
      chartData.reduce((acc, d) => acc + d.roi, 0) / chartData.length;
      ...
      <div className="bg-white rounded-2xl p-6">
        <div className="flex items-center justify-center h-[360px]">
          <div className="text-red-500">
            Failed to load chart data. Showing fallback data.
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <div>
          <button className="border rounded-xl px-3 sm:px-4 py-2 text-sm sm:text-base font-medium text-gray-700 flex items-center gap-2">
            {periodDisplay}
            <svg
